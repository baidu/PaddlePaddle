...
template <typename T>
class Fill_GradCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
#ifdef __HIPCC__
    const int64_t kMaxBlockDim = 256;
...
REGISTER_OP_CUDA_KERNEL(fill_inplace_grad, ops::Fill_GradCUDAKernel<float>,
                        ops::Fill_GradCUDAKernel<double>,
                        ops::Fill_GradCUDAKernel<int>,
                        ops::Fill_GradCUDAKernel<int64_t>,
                        ops::Fill_GradCUDAKernel<plat::float16>,
                        ops::Fill_GradCUDAKernel<bool>);
